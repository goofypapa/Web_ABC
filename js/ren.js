...
for(var i=0;i<listRen.length;i++){
    $('.imgList').append($($('#template1').html().replace('$urlRen$','https://www.goofypapa.com/app/ABC/ren/'+arr[num]+'/'+listRen[i]).replace('$classRen1$','dv'+listRen.length)));
    $('.audioAll').append($($('#template2').html().replace('$audio$','https://www.goofypapa.com/app/ABC/audio/'+audioRen[i])));
    arrRandom[i]=i+1;
}
